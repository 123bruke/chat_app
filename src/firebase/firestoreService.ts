...
export async function getOrCreateConversation(
  ...
  try {
    ...
    // Build an id from both participants so it's fairly predictable.
...
export async function sendMessage(
  ...
  try {
    ...
    batch.update(convRef, {
      ...
      [`unreadCounts.${recipientId}`]: 1, // bump the unread badge for the recipient
    });
...
export async function markConversationAsRead(
  ...
  try {
    ...
    // Non-fatal: the document might not be synced yet.
